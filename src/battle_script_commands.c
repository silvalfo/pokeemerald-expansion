...
const u16 sLevelCaps[NUM_SOFT_CAPS] = { 15, 23, 32, 42, 50, 60, 68, 75, 85, 95 };
const double sLevelCapReduction[9] = { .01, .00, .00, .00, .00, .00, .00, .00, .00};
const double sRelativePartyScaling[27] =
{
	3.00, 2.75, 2.50, 2.33, 2.25,
	2.00, 1.80, 1.70, 1.60, 1.50,
	1.40, 1.30, 1.20, 1.10, 1.00,
	0.95, 0.90, 0.87, 0.83, 0.75,
	0.70, 0.67, 0.62, 0.60, 0.57,
	0.55, 0.52,
};
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if ((IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN) || atkAbility == ABILITY_SUNBRINGER)
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_DEF_SPDEF_DOWN: // Close Combat
                ...
                break;
			case MOVE_EFFECT_DEFENSE_DOWN: // Strength
				if (!NoAliveMonsForEitherParty())
				{
					BattleScriptPush(gBattlescriptCurrInstr + 1);
					gBattlescriptCurrInstr = BattleScript_DefenseDown;
				}
				break;
            ...
            case MOVE_EFFECT_FLAME_BURST:
                if (IsBattlerAlive(BATTLE_PARTNER(gBattlerTarget))
						...
						&& GetBattlerAbility(BATTLE_PARTNER(gBattlerTarget)) != ABILITY_CRYSTAL_GUARD)
                {
...
static bool32 TryKnockOffBattleScript(u32 battlerDef)
{
        ...
        && !NoAliveMonsForEitherParty())
    {
        ...
        else
        {
            ...
            if ((gBattleMons[battlerDef].ability != ABILITY_GORILLA_TACTICS) && (gBattleMons[battlerDef].ability != ABILITY_ONE_TRICK_PONY))
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            ...
            {
                if (gProtectStructs[gBattlerTarget].spikyShielded && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD  && GetBattlerAbility(gBattlerAttacker) != ABILITY_CRYSTAL_GUARD)
                {
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
            if (gHitMarker & HITMARKER_OBEYS
             && (HOLD_EFFECT_CHOICE(holdEffectAtk) || (GetBattlerAbility(gBattlerAttacker) == ABILITY_GORILLA_TACTICS) || (GetBattlerAbility(gBattlerAttacker) == ABILITY_ONE_TRICK_PONY))
...
static void Cmd_switchineffects(void)
{
        ...
        && (gBattleMons[gActiveBattler].hp != gBattleMons[gActiveBattler].maxHP || gBattleMons[gActiveBattler].status1 != 0 || B_HEALING_WISH_SWITCH < GEN_8))
    {
		...
		&& GetBattlerAbility(gActiveBattler) != ABILITY_CRYSTAL_GUARD
...
static void Cmd_weatherdamage(void)
{
    ...
    gBattleMoveDamage = 0;
    if (IsBattlerAlive(gBattlerAttacker) && WEATHER_HAS_EFFECT && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_CRYSTAL_GUARD)
    {
...
static u8 AttacksThisTurn(u8 battlerId, u16 move) // Note: returns 1 if it's a charging turn, otherwise 2
{
    ...
    if (gBattleMoves[move].effect == EFFECT_SOLAR_BEAM
        && (IsBattlerWeatherAffected(battlerId, B_WEATHER_SUN) || GetBattlerAbility(gBattlerAttacker) == ABILITY_SUNBRINGER))
        return 2;
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        else
        {
            if ((!(gBattleWeather & B_WEATHER_ANY) || !WEATHER_HAS_EFFECT || GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_UTILITY_UMBRELLA)
                ...
                gBattleMoveDamage = gBattleMons[gBattlerAttacker].maxHP / 2;
            else if (gBattleWeather & B_WEATHER_SUN || GetBattlerAbility(gBattlerAttacker) == ABILITY_SUNBRINGER)
                gBattleMoveDamage = 20 * gBattleMons[gBattlerAttacker].maxHP / 30;
